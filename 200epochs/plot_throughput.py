...
def plot_gpu_utilization(data_file, output_file=None, time_unit='s'):
    ...
    ax.plot(df['time_sec'], df['gpu_util'], 'b-', linewidth=1)
    ax.set_ylabel('Compute throughput (%)')
    ax.set_xlabel(f'Time ({time_unit})')
    ax.set_ylim(0, 100)
    avg_gpu = df['gpu_util'].mean()
    ax.axhline(y=avg_gpu, color='r', linestyle='--', label=f'average: {avg_gpu:.1f}%')
    ax.set_title('GPU Compute Throughput Utilization')
    ax.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Plot GPU compute throughput from monitoring data.')
    ...
    parser.add_argument('-t', '--time_unit', default='s', help='Time unit label (default: s)')
