...
def main():
    ...
    matrix_worker = AdvancedMatrixWorker(size=4000, num_matrices=200, max_concurrent_transfers=2)
    ...
    get_next_batches, shutdown_prefetcher = create_batch_prefetcher(
        train_images, train_labels, batch_size, prefetch_epochs=2)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0 or epoch == num_epochs - 1:  # Print every 10 epochs and on the final epoch
            print(f"Epoch {epoch}:")
            ...
            print(f"Epoch {epoch}: train_acc={train_acc:.4f}, test_acc={test_acc:.4f}, time={epoch_time:.2f}s")
