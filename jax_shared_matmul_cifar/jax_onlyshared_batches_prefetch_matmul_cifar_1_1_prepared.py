...
def main():
    ...
    get_next_batches, shutdown_prefetcher = create_cached_batch_loader(
        cache_dir=batch_cache_dir, prefetch=1)
    
    # Create matrix worker for background computations
    matrix_worker = EnhancedMatrixWorker(
        size=4000, 
        num_matrices=200, 
    ...
    # Start matrix worker in background thread
    matrix_results_queue = Queue()
    
    def matrix_background_worker():
        results = matrix_worker.run()
        matrix_results_queue.put({'event': 'all_complete', 'results': results})
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0 or epoch == num_epochs - 1:
            ...
            print(f"Epoch {epoch}: test_acc={test_acc:.4f}, time={epoch_time:.2f}s")
    
    # Clean up
    shutdown_prefetcher()
    training_time = time.time() - training_start
    
    # Wait for matrix worker to complete
    try:
        matrix_result = matrix_results_queue.get(timeout=5)
        if matrix_result['event'] == 'all_complete':
            print("Matrix computations completed successfully")
            matrix_results = matrix_result.get('results', [])
            if matrix_results:
                avg_tflops = sum(r['tflops'] for r in matrix_results) / len(matrix_results)
                print(f"Average matrix multiplication performance: {avg_tflops:.2f} TFLOPS")
    except Exception as e:
        print(f"Error waiting for matrix results: {e}")
